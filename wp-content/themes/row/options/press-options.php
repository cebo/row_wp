...
$meta_box_press = array(
	'id' => 'CUSTOM FIELDS',
	'title' => 'Additional Variations For Press',
...
// Add meta box
function mytheme_press_add_box() {
	global $meta_box_press;
	foreach ( array( 'press-releases','dolo' ) as $page )
	add_meta_box($meta_box_press['id'], $meta_box_press['title'], 'mytheme_press_show_box', $page, $meta_box_press['context'], 			$meta_box_press['priority']);
}
// Callback function to show fields in meta box
function mytheme_press_show_box() {
...
jQuery(document).ready(function() {
	...
	echo '<input type="hidden" name="mytheme_press_meta_box_press_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';
...
add_action('save_post', 'mytheme_press_save_data');
// Save data from meta box
function mytheme_press_save_data($post_id) {
	global $meta_box_press;	
	// verify nonce
	if (!wp_verify_nonce($_POST['mytheme_press_meta_box_press_nonce'], basename(__FILE__))) {
		return $post_id;
	}
	// check autosave
